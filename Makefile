...
SERVER_BUILD := team-kirby-clash-deluxe
endif
...
ifeq ($(wildcard .env),)
	...
	go get -u
	go mod tidy
	go build -ldflags "-X 'main.serverBuildString=$(BUILD_STRING)'" -o ./build/team-kirby-clash-deluxe

docker:
	docker build -t team-kirby-clash-deluxe --build-arg BUILD_STRING="$(BUILD_STRING)" .
